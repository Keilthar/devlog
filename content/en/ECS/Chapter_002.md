...
partial struct SYS_Unit_Spawn : ISystem
{
    public void OnCreate(ref SystemState state)
    {
        ...
        // Adds the Components that define this entity
        EM.SetName(UnitEntity, "Unit");
        EM.AddComponent(UnitEntity, new LocalTransform());
        EM.AddComponent(UnitEntity, new TAG_Unit());
        // Remaining useful components
    }
}
```

I think this implementation reflects that duality quite well, between the raw declaration of the technical entity, which is essentially just a kind of empty container that only exists through its unique identifier, and the **Components** then added through the **EntityManager** orchestrator, which give it its functional identity.
...
Taken together, **Components** form a kind of **public data bus** that **Systems** read from and write to in order to produce processing. In other words, a **System** is blind to the progress or internal state of any specific entity, it only interacts through generically exposed data. That means the entity must carry, through its data, its current state and functional logic so that the relevant **System** can do its job.
...
And that's something extremely divisive, because OOP tends to make **objects autonomous and closed-off** (which is, incidentally, one of the fundamentals of Clean Code, even if how "clean" that is remains very debatable...).
...
Let's imagine, for example, that I don't want to destroy my units when they die, but simply move them off the map and disable them until their next respawn. Well, I could simply disable their `TAG_Unit` **Component** when their HP reaches 0, and then, when I need them again, **Query** them through:
...
A **System**, by construction, is optimised to **manipulate only strict data types** (so forget Nullable, forget strings, in short, forget managed objects).
...
partial struct SYS_Unit_Move : ISystem
{
    ...
    partial struct Job_UnitMove : IJobEntity
    {
        ...
        // Query on the manipulated components, with an additional strict ReadWrite/ReadOnly notion
...
I find that to be a very practical mental model, because if you've already written SQL queries, you immediately adopt certain reflexes about how to compartmentalise data into logical business tables.
...
I can create a singleton reference entity that carries the `NormalSpeed`. And personally, I name these singleton reference structures with the `REF_` prefix.
...
public struct COMP_Unit_Movement : IComponentData // represents movement capability at time T
{
    public float3 Direction;
    public float SpeedMultiplier;
}

public struct REF_Unit_Movement : IComponentData // unique singleton shared by all units
{
...
partial struct SYS_Unit_Move : ISystem
{
    ...
    partial struct Job_UnitMove : IJobEntity
    {
        ...
        // Query, with an added strict ReadWrite / ReadOnly notion on Components
        public void Execute(RefRW<LocalTransform> Transform, RefRO<COMP_Unit_Movement> COMP_Movement)
        {
            COMP_Unit_Movement UnitMovement = COMP_Movement.ValueRO;
            Transform.ValueRW.Position += DeltaTime
                                            * REF_Movement.NormalSpeed // unique reference speed that never changes
                                            * UnitMovement.SpeedMultiplier // time-varying multiplier for each unit, 1 by default
...
And that's the advantage compared with a constant: I can modify the value of the `REF_Unit_Movement` component live without restarting the game by changing code (and on top of that it makes tuning easier and allows units to evolve during the game).
...
Designing components and entities takes some time before you find the right balance, the right way to reason in terms of logical systems. Personally, I like the KISS approach (Keep It Stupidly Simple), and I work on the principle that a **System** should only handle a very limited action, which makes it easier to debug and naturally simplifies **Component** design:
- what does a system that moves my units need? -> naming convention for associated components: my `SYS_Unit_Movement` will manipulate `*_Unit_Movement_*` **Components**. If I can access unit health from this system, there's probably something wrong
- what data is shared or needs to be reset to a defined value? -> **Reference singleton**
- what data is specific to each unit? -> **Component** on the entity
- is that data repeatable? -> **IBuffer** vs **IComponent**
